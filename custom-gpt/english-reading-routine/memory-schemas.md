...
`Import Key` is a persistent identifier automatically generated by the storage backend. Do not include this field in a constructed entry unless the user explicitly instructs you to do so and provides its value.
...
| Notes | User-provided notes. Include this field only when the user explicitly provides or requests its content; otherwise omit it. |
| Import Key | Do not include this field unless the user explicitly asks and provides its value. |
...
| Custom Note | A customed open field. It may serve as a user note or other use. Include this field only when the user explicitly provides or requests its content; otherwise omit it. |
