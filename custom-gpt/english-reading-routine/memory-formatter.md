...
- Construct memory entries from objects provided by the user according to `memory-schemas.md`.
- Store the constructed entries through the `readingMemoryStorage` GPT Action.
...
2. Call the configured `readingMemoryStorage` GPT Action with the constructed canonical memory entries:

   ```json
   {
     "entries": [
       "<constructed canonical memory entry>",
       "<constructed canonical memory entry>"
     ]
   }
   ```
